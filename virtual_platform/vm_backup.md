...
（1） 必须到下一次全备任务完成之后，才开始删除过期的数据\(超过数据保留期限的数据\)；
  
（2） 如果新的备份失败，则过期的数据不删除\(超过数据保留期限的数据\)；
...
![说明: 1](/assets/V6.120181225185921.png)
