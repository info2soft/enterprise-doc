...
**注意事项：**
* 创建复制规则时开启了CDP，如果后续修改备份目录时，CDP会重新生成一个新的baseline，原CDP继续保留，CDP恢复时，可以选择不同时间点恢复到不同的时刻，选择不同的时间点，目录结构不同
* 关闭CDP选项时，已经生成的baseline会被删除

通过如下页面设置CDP数据的保存策略：

![](/assets/V6.118042612.png)
